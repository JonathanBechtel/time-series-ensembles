...
def check_residuals(series):
    ...
    return pd.DataFrame(
        [acorr_ljungbox(series[col])["lb_pvalue"] for col in series.columns]
    )


def generate_residuals_reports(
    errors_df: pd.DataFrame, hierarchical=False, threshold=0.05
) -> pd.DataFrame:
...
def build_lookback_predictions(
    forecaster, data, splitter, max_window_length: int = 10, hierarchical: bool = True
) -> pd.DataFrame:
    ...
    for idx, i in tqdm(enumerate(range(1, max_window_length))):
        ...
        if (
            "TransformedTargetForecaster__RecursiveTabularRegressionForecaster__window_length"
            in mod_params
        ):
            forecaster.set_params(
                TransformedTargetForecaster__RecursiveTabularRegressionForecaster__window_length=i
            )

        # get out-of-sample results for each window length
        tmp_results = evaluate(
            forecaster, y=data, cv=splitter, strategy="refit", return_data=True
        )
...
def extract_train_and_test_preds(
    results_df: pd.DataFrame, hierarchical: bool = False
) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {"y_true": y_true, "y_pred": y_pred, "Date": results_df["cutoff"].values}
    )
...
def calc_metrics_for_preds_df(preds_df: pd.DataFrame, round_vals=False) -> pd.DataFrame:
    ...
    for col in pred_cols:
        pred_metrics = calculate_metrics(
            preds_df["y_true"], preds_df[col], round_vals=round_vals
        )
...
def calc_ensembled_predictions(lookback_preds_df: pd.DataFrame) -> pd.DataFrame:
    ...
    pred_cols = sorted([col for col in lookback_preds_df.columns if "pred" in col])
    ...
    for i in range(2, len(pred_cols) + 1):
        ensemble_preds_df[f"ensemble_preds_{i}"] = lookback_preds_df.loc[
            :, pred_cols[:i]
        ].mean(axis=1)
...
def run_experiment(
    ...
    if not os.path.isfile(file_path):
        raise Exception(
            f"""Provided filepath does not exist on your computer: {file_path}.  
    ...
    print(
        f"""Loaded in the file from path: {file_path}, 
    ...
    lookback_preds = build_lookback_predictions(
        model,
        df[:510],
        splitter=splitter,
    ...
    residual_reports = generate_residuals_reports(
        ensemble_errors, hierarchical=hierarchical
    )
    ...
    fig1 = px.line(
        x=range(range_start, max_window_length),
    ...
    residual_reports.to_csv(
        residual_reports_path, index=True if hierarchical else False
    )
